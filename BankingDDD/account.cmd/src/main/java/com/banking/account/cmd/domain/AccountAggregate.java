...
import com.banking.account.common.events.AccountOpnendEvent;
...
public class AccountAggregate extends AggregateRoot {
    ...
    public AccountAggregate(OpenAccountCommand command){
        raiseEvent(AccountOpnendEvent.builder()
                .id(command.getId())
    ...
    public void apply(AccountOpnendEvent event){
        this.id = event.getId();
        this.active = true;
    ...
    public void depositFunds(double amount){
        ...
        if (amount <= 0){
            throw new IllegalStateException("Deposito no puede ser menor o igual a zero");
        }
